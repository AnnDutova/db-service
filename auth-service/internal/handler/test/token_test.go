...
func TestTokens(t *testing.T) {
	...
	t.Run("Invalid request", func(t *testing.T) {
		...
		mockTokenService.AssertNotCalled(t, "NewToken")
	})

	t.Run("Invalid token", func(t *testing.T) {
		invalidTokenString := "invalid"
		mockErrorMessage := "authProbs"
		mockError := model.UnauthorizedError(mockErrorMessage)
	...
	t.Run("Failure to create new token pair", func(t *testing.T) {
		...
		mockError := model.UnauthorizedError("Invalid refresh token")
		newPairArgs := mocks.Arguments{
			mocks.AnythingOfType("*context.emptyCtx"),
			mockUserResp,
		...
		mockTokenService.AssertCalled(t, "NewToken", newPairArgs...)
	})
